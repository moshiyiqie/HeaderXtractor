...
import os
import re
inputFilePath = 'D:/CRF++/result-on-pub-v1/result-pub.txt'
outputFilePath = 'D:/CRF++/result-on-pub-v1/out.csv'
print 'inputFilePath:',inputFilePath
print 'outputFilePath',outputFilePath

lines = open(inputFilePath).readlines()
dic={}
#labelset = ['<author>', '<title>', '<address>', '<email>', '<affiliation>', '<other>']
labelset = ['<web>', '<keyword>', '<author>', '<title>', '<abstract>', 
			'<pubnum>', '<date>', '<address>', '<email>', '<phone>', 
			'<page>', '<note>', '<affiliation>', '<degree>', '<intro>','<other>','<unknow>']
